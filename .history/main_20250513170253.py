...
def initialize_mt5_connection(config, logger):
    ...
    try:
        ...
        # Make sure MT5 is not running before we start it
        import subprocess
        import time
        import psutil
        
        # Kill all existing terminal64.exe processes
        try:
            for proc in psutil.process_iter(['pid', 'name']):
                if proc.info['name'] == 'terminal64.exe':
                    logger.info(f"Terminating existing MetaTrader 5 process (PID: {proc.info['pid']})...")
            ...
            for proc in psutil.process_iter(['pid', 'name']):
                if proc.info['name'] == 'terminal64.exe':
                    logger.info(f"Forcefully killing MetaTrader 5 process (PID: {proc.info['pid']})...")
                    psutil.Process(proc.info['pid']).kill()
            
            time.sleep(2)
        except Exception as e:
            logger.warning(f"Error managing existing MT5 processes: {e}")
        
        # Start MT5 as administrator with the /portable flag
        ...
        try:
            ...
            time.sleep(30)  # Give the terminal more time to start
        except Exception as e:
            logger.warning(f"Error starting MT5 with admin privileges: {e}")
            # Try regular start method as fallback
            logger.info("Falling back to regular start method...")
            subprocess.Popen([path])
            logger.info("Waiting 30 seconds for MetaTrader 5 terminal to start...")
            time.sleep(30)
        
        # Try to initialize with a much higher timeout value
        logger.info("Attempting to initialize MT5 connection with extended timeout...")
        for attempt in range(3):
            logger.info(f"Initialization attempt {attempt+1}/3")
            
            # Shut down any existing connection first for a clean start
            try:
                mt5.shutdown()
                time.sleep(2)
            except:
            ...
            # Try initialization with a very high timeout
            if mt5.initialize(path=path, login=login, password=password, server=server, timeout=120000):
                logger.info(f"MT5 initialized successfully on attempt {attempt+1}.")
                break
            else:
                error_code = mt5.last_error()
                logger.error(f"MT5 initialize() failed on attempt {attempt+1}, error code = {error_code}")
                
                if attempt < 2:
                    logger.info(f"Waiting before retry ({attempt+1}/3)...")
                    time.sleep(10)
        else:
            # All attempts failed
            logger.error("All MT5 initialization attempts failed. Giving up.")
            try: mt5.shutdown()
            except: pass
            return False

        # Verify connection and account information
        logger.info(f"MT5 initialized. Verifying connection...")
