...
def initialize_mt5_connection(config, logger):
    ...
    try:
        ...
        # Try to launch the MT5 terminal if it's not already running
        import subprocess
        import time
        
        # Check if MT5 terminal process is running
        try:
            # Using tasklist to check if terminal64.exe is running
            result = subprocess.run(['tasklist', '/FI', 'IMAGENAME eq terminal64.exe'], 
                                  capture_output=True, text=True)
            if 'terminal64.exe' not in result.stdout:
                logger.info("MetaTrader 5 terminal is not running. Attempting to start it...")
                # Start the terminal and wait for it to initialize
                subprocess.Popen([path])
                logger.info("Waiting 20 seconds for MetaTrader 5 terminal to start...")
                time.sleep(20)  # Give the terminal time to start
        except Exception as e:
            logger.warning(f"Error checking/starting MT5 terminal process: {e}")
        
        # Now try to initialize with a higher timeout value
        if not mt5.initialize(path=path, login=login, password=password, server=server, timeout=60000):
