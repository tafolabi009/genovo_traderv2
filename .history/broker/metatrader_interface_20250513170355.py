...
class MetatraderInterface:
    ...
    def get_recent_bars(self, symbol, timeframe, num_bars):
        ...
        try:
            ...
            chunk_size = min(num_bars, self.MAX_BARS)
            logger.debug(f"Requesting {chunk_size} recent bars for {symbol}")
            
            # Request the bars
            bars = mt5.copy_rates_from_pos(symbol, timeframe, 0, chunk_size)
            ...
            if num_bars > self.MAX_BARS:
                logger.debug(f"Fetching additional bars for {symbol} in chunks")
                ...
                while remaining_bars > 0:
                    ...
                    remaining_bars -= len(additional_df)
                    current_pos += len(additional_df)
                    
                    if len(additional_df) < chunk_size:
                        # Fewer bars returned than requested, no more available
                        break
            
            logger.debug(f"Successfully retrieved {len(df)} bars for {symbol}")
            return df
